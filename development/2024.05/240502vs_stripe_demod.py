...
def orientation(pfc,T):
    ...
    for q,n in zip(qs,range(len(qs))):
        ...
        qn = np.zeros((2,pfc.xRes,pfc.yRes))
        qn[0] = q[0]+delta_q[0]
        qn[1] = q[1]+delta_q[1]

        # Normalize
        qn = qn/np.sqrt(qn[0]**2+qn[1]**2)

        Q[0] += prob*(qn[0]**2-1/2)
        Q[1] += prob*qn[0]*qn[1]
        Q[2] += prob*(qn[1]**2-1/2)

        # Q[0] += prob*((q[0]+delta_q[0])*(q[0]+delta_q[0])-1/2)
        # Q[1] += prob*(q[0]+delta_q[0])*(q[1]+delta_q[1])
        # Q[2] += prob*((q[1]+delta_q[1])*(q[1]+delta_q[1])-1/2)
