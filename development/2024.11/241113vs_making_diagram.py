...
for i in range(len(Delta_Bs)):
    ...
    for m in range(len(strains)):
        if not finished_with_Delta_B:
            ...
            trailing_d2Fdt2 = np.array([1.1 if i % 2 == 0 else -1.1 for i in range(100)])
            ...
            while not free_energy_stable and not free_energy_sharp_drop and not finished_with_Delta_B:
                ...
                trailing_d2Fdt2 = np.roll(trailing_d2Fdt2, -1)
                trailing_d2Fdt2[-1] = d2Fdt2

                number_of_positive_trailing_d2Fdt2 = np.sum(trailing_d2Fdt2 > 0)

                tqdm.write(f"\rTime: {time_to_nucleation[j,i]:10.0f}, dFdt: {dFdt_new:.16f}, Number of trailing positive energy second diffs: {number_of_positive_trailing_d2Fdt2:2.0f} ", end='')
                
                if number_of_positive_trailing_d2Fdt2 == 100:    
                ...
                if number_of_positive_trailing_d2Fdt2  == 0 or (initial_free_energy-free_energy_new)/abs(initial_free_energy) > 0.1:
