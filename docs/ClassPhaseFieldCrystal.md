...
The fourier transform of a field which has a Bravais lattice symmetry will itself have a lattice symmetry, which is called the reciprocal lattice, and is useful to express the field in terms of a Fourier series.
...
The structure can alternatively be expressed in terms of the reciprocal lattice vectors (RLVs), which are generated by two *primitive* RLVs $\lbrace\boldsymbol{q}^{(n)}\rbrace_{n=1}^d$, satisfying
...
$$
\textrm{The primitive BLV-RLV orthogonality relation.}
$$
...
!!! example "Example: The 1D PFC - Crystal symmetry"
    ...
    Cutting the series off at the first mode is called the *one mode approximaion*, and the ground state of the 1D PFC is quite well approximated by this. 
    We denote the first set of reciprocal vectors $\mathcal R_{\textrm{per}}^{(1)}$ as the primary RLVs, and the first set of Bravais lattice vectors $\mathcal B_{\textrm{per}}^{(1)}$ as the primary BLVs.
...
!!! example "Example: The 1D PFC - Ground state"
    ...
    $$
    \frac{\partial \langle \tilde f(\psi^{\textrm{eq}}) \rangle}{\partial A} = 2 A (1-q^2)^2 + 2 \texttt{r} A + 6 \psi_0^2 A + 6A^3 = 0,
    $$
    
    which gives $A=0$ or

    $$
    (1-q^2)^2 + \texttt{r} + 3 \psi_0^2 + 3A^2 = 0,
    $$

    $$
    A^2 = (-\texttt{r} - (1-q^2)^2)/3  - \psi_0^2 
    $$

    $$
    A = \pm \sqrt{-\frac{1}{3} \left( - \texttt{r} - 3 \psi_0^2 - (1-q^2)^2 \right)}
    $$

    From the free energy density, however, it is easy to see that it is minimized by $q=1$, so we get
...
$$
\textrm{The PFC evolution equation}
$$
...
$$
\textrm{The unconserved PFC evolution equation}
$$

$$
\texttt{evolve_PFC_unconserved}
$$
...
$$
\textrm{The PFC evolution at mechanical equilibrium}
$$

$$
\texttt{evolve_PFC_mechanical_equilibrium}
$$
...
$$
\textrm{The hydrodynamic PFC model}
$$

$$
\texttt{evolve_PFC_hydrodynamic}
$$
